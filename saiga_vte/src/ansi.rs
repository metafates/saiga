...
impl<T: Timeout> Processor<T> {
    ...
    /// update.
    fn stop_sync_internal(&mut self, handler: &mut impl Handler, bsu_offset: Option<usize>) {
    ...
    #[cold]
    fn advance_sync(&mut self, handler: &mut impl Handler, bytes: &[u8]) -> usize {
    ...
    fn advance_sync_csi(&mut self, handler: &mut impl Handler, new_bytes: usize) {
    ...
    T: Timeout,
{
    ...
    fn esc_dispatch(&mut self, intermediates: &[u8], _ignore: bool, byte: u8) {
        ...
        match (byte, intermediates) {
            (b'B', intermediates) => configure_charset!(Charset::Ascii, intermediates),
            ...
            (b'0', intermediates) => {
                configure_charset!(Charset::SpecialCharacterAndLineDrawing, intermediates)
            }
...
#[inline]
fn attrs_from_sgr_parameters(handler: &mut impl Handler, params: &mut ParamsIter<'_>) {
